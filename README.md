...
### Быстрый старт (веб-приложение + API)

Одна команда запускает и **веб-интерфейс** (http://127.0.0.1:8000/), и **REST API**:
...
| Параметр | Описание |
|----------|----------|
| `.\run.ps1 -Gpu` | Использовать GPU для VLM |
| `.\run.ps1 -NoPreload` | Быстрый старт (модель загрузится при первом запросе) |
| `.\run.ps1 -Reload` | Автоперезагрузка при изменении кода (разработка) |

### REST API (FastAPI) — ручной запуск
...
| GET | `/api` | Информация об API и ссылки |
